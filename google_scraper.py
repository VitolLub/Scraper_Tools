...
#         json_data.append({
#             'name': random.choice(random_name) + ' ' + random.choice(random_surname),
#             'email': random_email,
...
#         for line in res:
#             outfile.write(line['name'] + ',' + line['email'] + ',' + line['phone'] + '\n')
...
def main():
    with sync_playwright() as p:
        ...
        for listing in listings:
            ...
            #         listing.locator(reviews_span_xpath).get_attribute('aria-label').split()[0].replace(',',
            #                                                                                            '.').strip())
            #     business.reviews_count = int(
            #         listing.locator(reviews_span_xpath).get_attribute('aria-label').split()[2].strip())
            # else:
